...
| Hardcoded admin credentials | Do not hardcode credentials in package code or seeders; provide via env or CLI |
...
**Security note:** weak credentials such as `admin@admin.com` / `admin` are allowed only when the user passes them explicitly via CLI (warning is shown).
